context("cash flow projection of ABSU")
library(vamc)
...
test_that("test for the cash flow projection", {
  expect_equal(projectABSU(testABSU, fundScen[1, 1:163, ], 1 / 12, pq, p,
  ...
  expect_equal(projectABSU(testABSU, fundScen[1, 1:163, ], 1 / 12, pq, p,
               ...
               42.49676, tolerance = 1)
})

test_that("Test for error message when pq, p, and df cannot cover maturity", {
  expect_error(projectABSU(testABSU, fundScen[1, 1:163, ], 1 / 12, c(1), p,
                           cForwardCurve)$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
  expect_error(projectABSU(testABSU, fundScen[1, 1:163, ], 1 / 12, pq, c(1),
                           cForwardCurve)$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
  expect_error(projectABSU(testABSU, fundScen[1, 1:163, ], 1 / 12, pq, p,
                           c(1))$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
})
