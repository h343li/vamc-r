context("cash flow projection of DBRP")
library(vamc)
...
test_that("test for the cash flow projection", {
  expect_equal(projectDBRP(testDBRP, fundScen[1, 1:163, ], 1 / 12, pq, p,
  ...
  expect_equal(projectDBRP(testDBRP, fundScen[1, 1:163, ], 1 / 12, pq, p,
               ...
               240.8832, tolerance = 1)
})

test_that("Test for error message when pq, p, and df cannot cover maturity", {
  expect_error(projectDBRP(testDBRP, fundScen[1, 1:163, ], 1 / 12, c(1), p,
                           cForwardCurve)$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
  expect_error(projectDBRP(testDBRP, fundScen[1, 1:163, ], 1 / 12, pq, c(1),
                           cForwardCurve)$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
  expect_error(projectDBRP(testDBRP, fundScen[1, 1:163, ], 1 / 12, pq, p,
                           c(1))$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
})
