context("cash flow projection of WBSU")
library(vamc)
...
test_that("test for the cash flow projection", {
  expect_equal(projectWBSU(testWBSU, fundScen[1, 1:163, ], 1 / 12, pq, p,
  ...
  expect_equal(projectWBSU(testWBSU, fundScen[1, 1:163, ], 1 / 12, pq, p,
               ...
               268.8288, tolerance = 1)
})

test_that("Test for error message when pq, p, and df cannot cover maturity", {
  expect_error(projectWBSU(testWBSU, fundScen[1, 1:163, ], 1 / 12, c(1), p,
                           cForwardCurve)$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
  expect_error(projectWBSU(testWBSU, fundScen[1, 1:163, ], 1 / 12, pq, c(1),
                           cForwardCurve)$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
  expect_error(projectWBSU(testWBSU, fundScen[1, 1:163, ], 1 / 12, pq, p,
                           c(1))$DA[1, 1],
               "df, pq, and p must have length > numStep from oneFundScen")
})
