...
### First 8 properties of the default DBRP
```{r, echo=FALSE, results='asis'}
# In this example, we will valuate testDBRP, a default data set for DBRP
knitr::kable(testDBRP[1:8])
# Again, we will arbitrarily use only the first scenarios to speed up the execution.
```

### Valuation of one variable annuity
```{r}
valuateOnePolicy(testDBRP, mortTable, fundScen[1:2, , ], 1 / 12, cForwardCurve)
```
...
ageOnePolicy(testDBRP, mortTable, histFundScen, histDates, dT = 1 / 12, targetDate, cForwardCurve)
```
